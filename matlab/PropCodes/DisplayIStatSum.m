...
figure
SI=zeros(1,nPropSteps);
for nPropStep=1:nPropSteps
    fracMom=IStatSum{nPropStep};
    SI(nPropStep)=sqrt(max(0,fracMom(2)-fracMom(1)));
    subplot(2,1,1)
    hold on
    if nPropStep<=nx
         plot(xx(nPropStep)/1000,SI(nPropStep),'ro')
    else
        plot(xx(nPropStep)/1000,SI(nPropStep),'bo')
    end
    subplot(2,1,2)
